...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (actCode.status === 'disabled') return json(res, 400, { error: 'Activation code is disabled' });
    if (actCode.status === 'expired' || isCodeExpired(actCode)) {
    ...
    if (isCodeExhausted(actCode)) return json(res, 400, { error: 'Activation code already used' });
